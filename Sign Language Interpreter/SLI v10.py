...
def main():
    ...
    while True:
        ...
        if key & 0xFF == ord('t'):
            mode="Training"
            training_label=4
            training_class="HANG LOOSE"
            
            build_resp_screen(rsp_img,wb_w,wb_h,cur_cx,cur_cy,resp_zoom,mode,training_label,training_class)
            
            ## IMPORTANT: Will need to ask the LABEL and CLASS here before start training. 
        ...
        if f_action=='c':
            cv2.putText(cap_img,"+++"+f_action,(wb_w-120,50),cv2.FONT_HERSHEY_PLAIN,3,(73,3,252),3)
            f_action=""
...
def build_resp_screen(rsp_img,w_size,h_size,x,y,resp_zoom,mode,training_label,training_class):
    ...
    if mode=="Interpreting":
        ...
        if (label_index>=0 and label_index<=len(labels_class)):
            cv2.putText(rsp_img,labels_class[label_index],(10,50),cv2.FONT_HERSHEY_SIMPLEX,0.7,(73,3,252),3)
